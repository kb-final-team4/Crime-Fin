...
public class LinkServiceImpl implements LinkService{
	...
	public void doLinkService(String newHistoryId, String newPhishingId, String memberId, String url) {
		String oldPhId = null;
		oldPhId = linkDAO.findOldPhishingIdForLink(memberId);
		//앞전 단계에서 번호 조회한적 없을때
		if(oldPhId==null) {
			...
			linkDAO.insertPhishingInfoWithLink(phishingInfoVO);
			...
			linkDAO.insertHistoryWithLink(historyVO);
		}
		//앞전 단계에서 번호 조회 했을때 
		else {
			PhishingInfoVO phishingInfoVO = new PhishingInfoVO(oldPhId, null, null, url);
			linkDAO.updatePhishingInfoWithLink(phishingInfoVO);
		}
	}
